...
function ContentEditDialog({ defaultValue, onOk, title, description, icon, secret }) {
  ...
  return <>
    ...
    <Mui.ListItemButton onClick={() => setState(true)}>
      ...
      <Mui.ListItemText primary={title} secondary={defaultValue ? (secret ? '************' : defaultValue) : 'Click to edit'} secondaryTypographyProps={{ textOverflow: 'ellipsis', overflow: 'hidden', width: '100%', maxHeight: '3em' }} />
    </Mui.ListItemButton>
