...
function EditDialog({ open, onClose, onOk, valueType, dialogType, defaultValue }) {
  ...
  const [themeMode, setThemeMode] = React.useState(theme.getCurrentThemeMode())

  React.useEffect(() => {
    theme.listenToThemeModeChange(m => setThemeMode(m))
  }, [])
    ...
    <Mui.DialogContent>
      <Mui.Grid container spacing={2}>
        ...
        <Mui.Grid item xs={12}>
          ...
          <ContentEditor height={'20vh'} width={'100%'} mode={themeMode} language={valueType === 'User Script' ? 'python' : 'markdown'} value={content} onChange={(value) => {
            setContent(value)
          }} />
        </Mui.Grid>
        <Mui.Grid item xs={12}>
...
function PanelExtraInfos({ raiseMessage }) {
  ...
  return <Mui.Box sx={{ height: '100%', overflowY: 'scroll' }}>
